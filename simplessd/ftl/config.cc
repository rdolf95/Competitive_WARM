...
const char NAME_GC_STAT_FILE[] = "GCStatFile";
const char NAME_REFRESH_STAT_FILE[] = "RefreshStatFile";
...
Config::Config() {
  ...
  randomSeed = 0;
  initEraseCount = 0;
  coolDownWindowSize = 20;

  gcStatFile = "./log/gcStat.txt";
  refreshStatFile = "./log/refreshStat.txt";
...
bool Config::setConfig(const char *name, const char *value) {
  ...
  else if (MATCH_NAME(NAME_GC_STAT_FILE)) {
    gcStatFile = value;
  }
  else if (MATCH_NAME(NAME_REFRESH_STAT_FILE)) {
    refreshStatFile = value;
  }
...
std::string Config::readString(uint32_t idx) {
	switch (idx) {
		case FTL_GC_STAT_FILE:
			return gcStatFile;
    case FTL_REFRESH_STAT_FILE:
      return refreshStatFile;
		default:
			return "";
	}
}
